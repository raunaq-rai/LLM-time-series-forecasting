  ...
  \begin{itemize}
    \item \textbf{Prey:}
      \begin{itemize}
        ...
        \item $R^2$ Score: -1.7590
      \end{itemize}
    \item \textbf{Predator:}
      \begin{itemize}
        \item Mean Squared Error (MSE): 0.1324
        \item Mean Absolute Error (MAE): 0.1904
        \item $R^2$ Score: -0.4884
      \end{itemize}
  \end{itemize}

  
  Both the predator prediction, however, is unstable, with a negative $R^2$ indicating performance worse than simply predicting the mean. This reflects the model’s lack of understanding of the underlying dynamics.
...
We trained the model for 5,000 steps using this setup, which was approximately 62\% of the total FLOP budget. Evaluation on a held-out validation set yielded a final loss of 0.2981 and a perplexity of 1.35.
...
\begin{table}[ht]
  ...
  \hline
  \textbf{Remaining FLOPs} & -- & $1.48 \times 10^{16}$ & \textbf{14.82\%} \\
  \hline
  \end{tabular}%
  }
  \vspace{0.2cm}
